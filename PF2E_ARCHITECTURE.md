...
8. **Prevent Test Memory Leaks.** When writing GdUnit4 tests, you MUST wrap manually instantiated Nodes (like `PFActor.new()` or `TurnManager.new()`) in `auto_free()` to ensure they are properly cleaned up and do not create orphan nodes.
