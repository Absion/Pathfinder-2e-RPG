...
5. **Open RPG Creative (ORC) License Compliance & Custom Pantheon:** While we are implementing the Pathfinder 2e rule mechanics, we MUST AVOID using Paizo's proprietary Intellectual Property (IP). Do not include specific Paizo deities (e.g., Desna, Sarenrae, Abadar), unique lore-specific ancestries, or setting-specific locations/factions that are not covered under the open ORC license. The game's divine pantheon is strictly comprised of **The Thirteen** original deities (`aethelis`, `kaldian`, `cankros`, `tavrin`, `brada`, `kragthor`, `morwenna`, `oneris`, `vurrok`, `severin`, `bathyos`, `stryvan`, `corvyna`).
6. **Mandatory Documentation:** Every class and significant method MUST be prefixed with Godot 4 `##` docstrings so the editor can generate rich tooltips and in-editor documentation. Single-letter variables (`w`, `s`, `x` outside of coordinates) are strictly forbidden to ensure legibility.
...
* **The Thirteen Pantheon (`deities`):** All divine options in `pf2e_data.db` strictly belong to **The Thirteen** (`category = 'The Thirteen'`). Every deity record maps to its domain arrays, cleric spell dicts, boons, curses, divine sanctification (`0`–`5`), and belief IDs (`edict_<name>` / `anathema_<name>`).
* **Ancestries & Languages (`ancestries`):** Ancestry descriptions use `description` (inherited from `PFEntity`) as their single source of truth (`ancestry_description` is deprecated). Bonus languages are not constrained per ancestry; characters with Intelligence modifier > 0 dynamically select bonus languages from all available game languages. Ancestries do not grant starting items (`granted_items` removed).
...
* **Beliefs & Moral Tenets (`beliefs`):** All edicts and anathemas reside in the single unified `beliefs` table (`id, name, type, mechanic_hook, description`).
  * **Naming Pattern:** Edict IDs MUST follow the `edict_<example>` pattern (e.g. `edict_study`, `edict_protect_weak`). Anathema IDs MUST follow the `anathema_<example>` pattern (e.g. `anathema_lies`, `anathema_undead`).
  * **Mechanical Hooks:** Deity edicts utilize `mechanic_hook = "deity_edict"` and deity anathemas utilize `mechanic_hook = "deity_anathema"` to enable dialogue managers, narrative engines, and class systems (Cleric/Champion) to track and enforce alignment choices and taboos in story actions.
