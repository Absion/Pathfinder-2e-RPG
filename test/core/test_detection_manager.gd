...
func test_default_detection_state():
	...
	assert_int(state).is_equal(PFCombatConstants.DetectionState.OBSERVED)
...
func test_relational_asymmetry():
	...
	assert_int(manager.get_detection_state(guard_1, rogue)).is_equal(PFCombatConstants.DetectionState.UNDETECTED)
	assert_int(manager.get_detection_state(guard_2, rogue)).is_equal(PFCombatConstants.DetectionState.OBSERVED)
	
	# Verify that rogue seeing Guard 1 is completely independent
	assert_int(manager.get_detection_state(rogue, guard_1)).is_equal(PFCombatConstants.DetectionState.OBSERVED)
...
func test_remove_actor_cleanup():
	...
	assert_int(manager.get_detection_state(guard_1, rogue)).is_equal(PFCombatConstants.DetectionState.HIDDEN)
	
	manager.remove_actor(rogue)
	# Should revert back to OBSERVED as it doesn't exist in the matrix anymore
	assert_int(manager.get_detection_state(guard_1, rogue)).is_equal(PFCombatConstants.DetectionState.OBSERVED)
