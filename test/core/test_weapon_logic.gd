...
func test_weapon_handedness() -> void:
	...
	var strike = PFActionStrike.new(weapon)
	assert_bool(strike.execute(attacker, defender)).is_false()
...
func test_two_hand_trait() -> void:
	...
	strike.execute(attacker, defender) # Base damage d8
	
	# Equip in two hands
	inv.equip_weapon(bastard_sword, 2)
	strike.execute(attacker, defender) # Should trigger Two-Hand and upgrade die
