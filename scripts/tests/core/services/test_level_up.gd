...
func test_level_up() -> void:
	...
	database.query("INSERT OR IGNORE INTO class_progressions (class_id, level, granted_features, granted_feat_slots, granted_spells) VALUES ('fighter', 2, '[]', '[\"class\", \"skill\"]', '{}')")
	...
	database.query("INSERT OR IGNORE INTO feats (id, name, feat_type, level, traits, prerequisites, granted_rules, description) VALUES ('toughness', 'Toughness', 3, 1, '[]', '{}', '{}', '');")
