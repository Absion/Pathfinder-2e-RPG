...
static func assert_eq(actual: Variant, expected: Variant, message: String = "") -> void:
	if actual != expected:
		push_error("TEST FAILED: %s. Expected %s, got %s." % [message, str(expected), str(actual)])
	else:
		print("  [OK] %s" % message)

static func run_all_tests() -> void:
	print("--- Running Monster Generator Tests ---")
	test_brute_generation()
	test_sniper_generation()
	test_elite_template()
	test_weak_template()
	print("--- Monster Generator Tests Complete ---")

static func test_brute_generation() -> void:
